...
def NowMusicCard(music_list: list[Music]) -> Card:
    ...
    playing_music_card = Card(theme=Types.Theme.INFO, color=Color(hex_color=__MUSIC_LIST_PLAYING_MUSIC_COLOR))
...
def searchCard(music_dict: dict) -> Card:
    ...
    for key in music_dict.keys():
        card = Card(color=Color(hex_color=ASSETS[key]['color']))
        search_list: list[Music] = music_dict[key]
        if search_list:
            for music in search_list:
                card.append(
                    Module.Section(
                        Element.Text(f'** ({music_list.index(music) + 1}) {music.name} - {music.author}**',
                                     type=Types.Text.KMD),
...
def channel_card(user_name, channel_name, cu_time, command=""):
    card = Card(theme=Types.Theme.INFO, size=Types.Size.LG)
    card.append(Module.Section(Element.Text(f"**用户名**\n{user_name}", type=Types.Text.KMD)))
    card.append(Module.Section(Element.Text(f"**频道**\n{channel_name}", type=Types.Text.KMD)))
    card.append(Module.Section(Element.Text(f"**{command}时间**\n{cu_time}", type=Types.Text.KMD)))
    return card
